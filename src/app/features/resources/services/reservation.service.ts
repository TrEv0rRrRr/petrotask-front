...
export class ReservationService {
  ...
  getAllReservations(): Observable<Reservation[]> {
    return this.http
      .get<ReservationResource[]>(this.baseUrl)
      .pipe(
        map((resources) =>
          resources.map((resource) =>
            ReservationAssembler.toEntityFromResource(resource)
          )
        )
      );
  }
  ...
  getReservationById(id: number): Observable<Reservation> {
    return this.http
      .get<ReservationResource>(`${this.baseUrl}/${id}`)
      .pipe(
  ...
  createReservation(reservation: Reservation): Observable<Reservation> {
    ...
    return this.http
      .post<ReservationResource>(this.baseUrl, createResource)
      .pipe(
        map((resource) => ReservationAssembler.toEntityFromResource(resource))
      );
  }
  ...
  getReservationsByResource(
    ...
    return this.http
      .get<ReservationResource[]>(
        `${this.baseUrl}/by-resource/${resourceType}/${resourceId}`
      )
      .pipe(
        map((resources) =>
          resources.map((resource) =>
