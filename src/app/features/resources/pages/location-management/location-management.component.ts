...
export class LocationManagementComponent implements OnInit {
  ...
  getLocationsForZone(zoneId: number): Location[] {
    ...
    if (this.statusFilter) {
      return zoneLocations.filter(
        (location) => location.status === this.statusFilter
      );
    }
  ...
  getActiveLocationCountForZone(zoneId: number): number {
    const zoneLocations = this.zoneLocationsCache.get(zoneId) || [];
    return zoneLocations.filter((location) => location.status === 'ACTIVE')
      .length;
  }
  ...
  createLocation(location: Location): void {
    ...
    this.locationService
      .addLocationToZone(location.zoneId, location)
      .pipe(finalize(() => (this.loading = false)))
      .subscribe({
        next: (createdLocation: Location) => {
          // Update cache
          const zoneLocations =
            this.zoneLocationsCache.get(location.zoneId) || [];
  ...
  updateLocation(location: Location): void {
    ...
    this.locationService
      .updateLocationStatus(location.id, location.status)
      .pipe(finalize(() => (this.loading = false)))
      .subscribe({
        next: (updatedLocation: Location) => {
          this.uiService.showSnackbar({
            message: 'Estado de ubicación actualizado exitosamente',
            type: 'success',
          });
          ...
          this.locationService
            ...
            .subscribe({
              next: (updatedLocation: Location) => {
                ...
                const zoneLocations =
                  this.zoneLocationsCache.get(location.zoneId) || [];
                const cacheIndex = zoneLocations.findIndex(
                ...
                this.uiService.showSnackbar({
                  message: `Ubicación ${actionKey}da exitosamente`,
                  type: 'success',
                });
              ...
              error: (error: any) => {
                ...
                this.uiService.showSnackbar({
                  message: 'Error actualizando estado de ubicación',
                  type: 'error',
                });
              },
            });
