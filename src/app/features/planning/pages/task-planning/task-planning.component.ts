...
export class TaskPlanningComponent implements OnInit {
  @ViewChild('scheduledActionsTemplate')
  scheduledActionsTemplate!: TemplateRef<any>;
  @ViewChild('unscheduledActionsTemplate')
  unscheduledActionsTemplate!: TemplateRef<any>;
  ...
  loadTasksForDate(date: Date): void {
    // Load all task programmings first
    this.taskProgrammingService.getAllTaskProgrammings().subscribe({
      next: (programmings) => {
        console.log('All programmings loaded:', programmings);

        // Filter programmings for the selected date
        this.taskProgrammings = programmings.filter((p) => {
          ...
          console.log(
            `Programming ${
              p.taskProgrammingId
            } date: ${programmingDate.toISOString()}, selected: ${date.toISOString()}, match: ${isSameDate}`
          );
          return isSameDate;
        });

        console.log(
          'Filtered programmings for selected date:',
          this.taskProgrammings
        );
        this.generateCalendarEvents();

        // Now load tasks and filter out scheduled ones
        this.loadUnscheduledTasks();
      },
      error: (error) => {
        console.error('Error loading task programmings:', error);
        this.snackBar.open('Error loading scheduled tasks', 'Close', {
          duration: 3000,
        });
      },
    });
  ...
  private loadUnscheduledTasks(): void {
    this.taskService.getAllTasks().subscribe({
      next: (tasks) => {
        ...
        // Get ALL scheduled task IDs (not just for today)
        const scheduledTaskIds = this.taskProgrammings.map((tp) => tp.taskId);
        console.log('Scheduled task IDs:', scheduledTaskIds);

        this.unscheduledTasks = tasks.filter(
          (task) => !scheduledTaskIds.includes(task.taskId)
        );
        console.log('Unscheduled tasks:', this.unscheduledTasks);
  ...
  onDragStart(event: DragEvent, task: Task): void {
    ...
    if (event.dataTransfer) {
      event.dataTransfer.setData(
        'application/json',
        JSON.stringify({ type: 'task', data: task })
      );
    }
  }
  ...
  scheduleTask(task: Task, timeSlot: string): void {
    ...
    this.taskProgrammingService.addTaskProgramming(newProgramming).subscribe({
      next: (createdProgramming) => {
        ...
        this.unscheduledTasks = this.unscheduledTasks.filter(
          (t) => t.taskId !== task.taskId
        );
        this.searchTasks();
        this.generateCalendarEvents();
        this.saveState();
        this.snackBar.open('Task scheduled successfully', 'Close', {
          duration: 3000,
        });
      },
      error: (error) => {
        console.error('Error scheduling task:', error);
        this.snackBar.open('Error scheduling task', 'Close', {
          duration: 3000,
        });
      },
    });
    ...
    this.taskProgrammingService
      ...
      .subscribe({
        next: (updatedProgramming) => {
          const index = this.taskProgrammings.findIndex(
            (tp) => tp.taskProgrammingId === programming.taskProgrammingId
          );
          if (index !== -1) {
          ...
          this.snackBar.open('Task rescheduled successfully', 'Close', {
            duration: 3000,
          });
        },
        error: (error) => {
          console.error('Error rescheduling task:', error);
          this.snackBar.open('Error rescheduling task', 'Close', {
            duration: 3000,
          });
        },
      });
  ...
  unscheduleTask(programming: TaskProgramming): void {
    ...
    this.taskService.getTaskById(programming.taskId).subscribe({
      next: (task) => {
        ...
        this.taskProgrammings = this.taskProgrammings.filter(
          (tp) => tp.taskProgrammingId !== programming.taskProgrammingId
        );
        ...
        this.snackBar.open('Task unscheduled successfully', 'Close', {
          duration: 3000,
        });
      },
      error: (error) => {
        console.error('Error unscheduling task:', error);
        this.snackBar.open('Error unscheduling task', 'Close', {
          duration: 3000,
        });
      },
    });
  ...
  cancelScheduling(programming: TaskProgramming): void {
    this.taskProgrammingService
      .updateTaskProgrammingStatus(programming.taskProgrammingId, 'CANCELLED')
      .subscribe({
        next: (updatedProgramming) => {
          const index = this.taskProgrammings.findIndex(
            (tp) => tp.taskProgrammingId === programming.taskProgrammingId
          ...
          this.snackBar.open('Task programming cancelled', 'Close', {
            duration: 3000,
          });
        },
        error: (error) => {
          console.error('Error cancelling task programming:', error);
          this.snackBar.open('Error cancelling task programming', 'Close', {
            duration: 3000,
          });
        },
      });
    ...
    this.taskProgrammingService
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.snackBar.open('Error updating task times', 'Close', {
            duration: 3000,
          });
        },
      });
  ...
  searchTasks(): void {
    if (!this.searchTerm.trim()) {
      ...
      this.filteredUnscheduledTasks = this.unscheduledTasks.filter(
        (task) =>
  ...
  getProgrammingById(id: string | number | undefined): TaskProgramming {
    return (
      this.taskProgrammings.find((tp) => tp.taskProgrammingId == id) ||
      new TaskProgramming()
    );
  }
