  ...
  implements OnInit, AfterViewInit, OnChanges, OnDestroy
{
  ...
  ngOnChanges(changes: SimpleChanges): void {
    ...
    if (
      ...
      !isEqual(changes['sort']?.currentValue, changes['sort']?.previousValue)
    ) {
  ...
  ngAfterViewInit(): void {
    ...
    if (!isUndefined(this.dataSource)) {
      ...
      this.filteredDataSource.filterPredicate = (data: any, filter: string) => {
   ...
   */
  initEvents(): void {
    this.matSort.sortChange.pipe(takeUntil(this.onDestroy)).subscribe(() => {
      const sort: Sort = {
